...
def test_create_todos(client, token, mock_db_time):
    with mock_db_time(Todo) as time:
...
async def test_list_todos_pagination_should_return_2_todos(session, user, client, token):
    expected_todos = 2
    session.add_all(TodoFactory.create_batch(expected_todos + 3, user_id=user.id))
    await session.commit()
...
async def test_list_todos_filter_title_should_return_5_todos(session, user, client, token):
    ...
    session.add_all(TodoFactory.create_batch(expected_todos, user_id=user.id, title='Test todo 1'))
    session.add_all(TodoFactory.create_batch(expected_todos, user_id=user.id))
...
@pytest.mark.asyncio
async def test_delete_other_users_todo(session, client, other_user, token):
    todo_other_user = TodoFactory(user_id=other_user.id)
    session.add(todo_other_user)
    await session.commit()

    response = client.delete(f'/todos/{todo_other_user.id}', headers={'Authorization': f'Bearer {token}'})
...
@pytest.mark.asyncio
async def test_get_todo(client, user, token, mock_db_time):
    with mock_db_time(Todo) as time:
        client.post(
            '/todos/',
            headers={'Authorization': f'Bearer {token}'},
...
def test_get_todo_error(client, token):
    response = client.get('/todos/14', headers={'Authorization': f'Bearer {token}'})
