...
def read_users(offset=0, limit=10, session: Session = Depends(get_session), current_user=Depends(get_current_user)):
...
def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), session: Session = Depends(get_session)):
    ...
    if not user:
        raise HTTPException(HTTPStatus.UNAUTHORIZED, detail='Incorrect email or password')
